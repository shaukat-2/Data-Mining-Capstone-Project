...
if(len(options_dishes)<=0 or len(options_dishes)> 3):
    check2 = 0
    st.sidebar.error("Please select at least one and at most three dishes.")
else:
    check2 = 1
...
option_zip = st.sidebar.selectbox(
     'Please select relevant ZIP Code:',
     zip_code_list)
if(len(option_zip) <= 0):
    check3=0
    st.error("Please select ZIP Code.")
else:
    check3=1
...
if submit:
    if(check4 == 1 & check1 ==1 & check2 ==1 & check3 == 1):
